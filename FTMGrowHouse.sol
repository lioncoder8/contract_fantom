...
contract FTMGrowHouse {
    ...
    constructor() {
        ...
        // addresses and instanxes in Fantom testnet
        // USDC_FTM_LP = IERC20(0xA7B8510fe299Bc69f8aF42406e1795319a9226d8);
        // USDC = IERC20(0x39968964A55901043368F18Ff14DbE2eE3B648ce);
        // WFTM = IERC20(0xf1277d1Ed8AD466beddF92ef448A132661956621);
        // uniswapV2Router = IUniswapV2Router02(0xa6AD18C2aC47803E193F75c3677b14BF19B94883);

        USDC_FTM_LP = IERC20(0x2b4C76d0dc16BE1C31D4C1DC53bF9B45987Fc75c);
    ...
    function stakeStableCoins(uint256 amtx, address ref) public {
        ...
        // how much FTM should we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        // capture the contract's current LP balance
        uint256 initialLPBalance = USDC_FTM_LP.balanceOf(address(this));

        // add liquidity to spookeyswap
        addLiquidity(otherHalf, newBalance);

        // how much LP should we stake
        uint256 newLPBalance = USDC_FTM_LP.balanceOf(address(this)).sub(initialLPBalance);
    ...
    function swapEthForTokens(uint256 ethAmount, address _to) internal {
        // generate the uniswap pair path of FTM -> USDC
    ...
    function swapTokensForEth(uint256 tokenAmount, address _to) internal {
        // generate the uniswap pair path of token -> wftm
        address[] memory path = new address[](2);
        path[0] = address(USDC);
    ...
    function stake(uint256 amtx, address ref) internal {
        ...
        USDC_FTM_LP.safeTransfer(owner, stakeFee);
    }
    ...
    function withdrawDivs() external returns (uint256 withdrawAmount) {
        ...
        USDC_FTM_LP.safeTransfer(msg.sender, adjustedAmt);
        USDC_FTM_LP.safeTransfer(owner, withdrawFee);
    ...
    function withdrawInitial(uint256 key) external {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ // LESS THAN 10 DAYS STAKED
            ...
            USDC_FTM_LP.safeTransfer(msg.sender, transferAmt);
            USDC_FTM_LP.safeTransfer(owner, minusAmt);
    ...
    function withdrawRefBonus() external {
        ...
        USDC_FTM_LP.safeTransfer(msg.sender, amtz);
    }
